...
-- Tabla: machine  
-- Se eliminó model_id; se mantiene sensor_id, name y description  
...
-- Tabla: alert  
-- Se cambia error_type a integer (1, 2 o 3)  
-- Se elimina cualquier columna extra y se renombra la FK hacia vibration_data como data_id
...
-- (limit_config es independiente, no requiere FK)
...
BEGIN
    -- Si el registro tiene severity = 2, verifica repeticiones en el intervalo
    IF NEW.severity = 2 THEN
        ...
        IF repeated_count >= 1 THEN
            -- Inserta alerta de tipo 3 (generada por software)
...
-- ============================================
-- 9) Optimización de índices y trigger de severidad
-- ============================================

-- Índices para la tabla vibration_data
...
-- Índice compuesto para el trigger en vibration_data
CREATE INDEX IF NOT EXISTS idx_vibration_severity_pattern 
ON public.vibration_data (sensor_id, severity, date DESC);
